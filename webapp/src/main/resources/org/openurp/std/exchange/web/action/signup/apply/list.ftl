...
[@b.grid items=exchangeApplies var="exchangeApply"]
  [@b.gridbar]
    ...
    bar.addItem("${b.text("action.export")}",
       action.exportData("std.code:学号,std.name:姓名,std.person.gender.name:性别,std.person.birthday:出生日期," +
  ...
  [@b.row]
    [@b.boxcol /]
    [@b.col width="9%" property="std.code" title="学号"/]
    [@b.col width="8%" property="std.name" title="姓名"][@b.a href="!info?id=${exchangeApply.id}"]${exchangeApply.std.name}[/@][/@]
